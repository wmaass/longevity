...
export async function fetchPGSFile(id) {
  ...
  // 1) Entpackte Version vorhanden?
  if (fs.existsSync(unpackedPath)) {
    const size = fs.statSync(unpackedPath).size;
    if (size > MAX_SIZE_MB * 1024 * 1024) {
      throw new Error(`Überspringe ${id}: Entpackte Datei zu groß (${(size / 1024 / 1024).toFixed(1)} MB)`);
    }
    console.log(`==> Verwende lokal entpackte PGS-Datei: ${unpackedPath}`);
    return fs.readFileSync(unpackedPath, 'utf8');
  }

  let gzBuffer;

  // 2) Gzipped-Version vorhanden?
  if (fs.existsSync(gzPath)) {
    const size = fs.statSync(gzPath).size;
    if (size > MAX_SIZE_MB * 1024 * 1024) {
      throw new Error(`Überspringe ${id}: Gz-Datei zu groß (${(size / 1024 / 1024).toFixed(1)} MB)`);
    }
    console.log(`==> Verwende lokale PGS-GZ-Datei: ${gzPath}`);
    gzBuffer = fs.readFileSync(gzPath);
  } else {
    // 3) Von FTP laden
    // HEAD-Request zur Prüfung der Dateigröße
    const headRes = await fetch(ftpUrl, { method: 'HEAD' });
    if (!headRes.ok) {
      throw new Error(`HEAD-Request fehlgeschlagen: ${headRes.statusText}`);
    }
    const contentLength = parseInt(headRes.headers.get('content-length') || '0', 10);
    if (contentLength > MAX_SIZE_MB * 1024 * 1024) {
      throw new Error(`Überspringe ${id}: Remote-Datei zu groß (${(contentLength / 1024 / 1024).toFixed(1)} MB)`);
    }

    // Jetzt erst: GET-Download
    console.log(`==> Lade PGS-Datei von FTP: ${ftpUrl}`);
    const res = await fetchWithRetry(ftpUrl);
    if (!res.ok) {
      throw new Error(`PGS-Datei ${id} konnte nicht von FTP geladen werden (${res.statusText})`);
    }
    gzBuffer = Buffer.from(await res.arrayBuffer());


    fs.writeFileSync(gzPath, gzBuffer);
    console.log(`==> PGS-Datei gespeichert unter: ${gzPath}`);
  }
  ...
  if (decompressedSize > MAX_SIZE_MB * 1024 * 1024) {
    throw new Error(`Überspringe ${id}: Entpackte Datei zu groß (${(decompressedSize / 1024 / 1024).toFixed(1)} MB)`);
  }
  ...
  for (const id of pgsIds) {
    ...
    try {
      ...
      if (lines.length > MAX_VARIANTS_ALLOWED) {
        console.warn(`⚠️  Überspringe ${id}: enthält ${lines.length} Zeilen (Limit: ${MAX_VARIANTS_ALLOWED})`);
        completedPGS++;
        continue;
      }
